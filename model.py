...
from sklearn.feature_extraction.text import CountVectorizer  # strings to numeric
...
df["no_punc"] = df["clean_text"].map(lambda x: "".join([w.strip(punctuation) for w in x]))

# combining all text into one string
corpus = "\n".join([t for t in df["no_punc"]])
